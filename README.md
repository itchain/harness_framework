...
- `CLAUDE.md` 템플릿 (**반드시 수정**)
- `git init` (main 브랜치)

를 설치한다. 이미 있는 파일은 덮어쓰기 전에 묻는다 (idempotent).

**설치 후**:

1. `CLAUDE.md`를 프로젝트 규칙에 맞게 수정 (기술 스택, CRITICAL 규칙, 명령어)
2. Claude Code 세션에서 첫 phase 시작:
   - 빈 레포 → `/harness-plan mvp`
   - 기존 코드에 기능 추가 → `/harness-plan feature`
   - 버그 픽스 → `/harness-plan patch`
   - 구조 개선 → `/harness-plan refactor`
...
mkdir -p docs phases
# 이후 CLAUDE.md를 직접 작성하고 git init
```

### 업데이트

하네스가 업데이트되면 프로젝트 루트에서 설치 커맨드를 다시 실행하면 된다 (덮어쓰기 확인 프롬프트가 뜸).
