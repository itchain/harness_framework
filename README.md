...
> 💡 **실전 대화 예시가 궁금하다면**: [SCENARIO.md](SCENARIO.md)에서 실제 명령 실행 시 어떤 대화가 오가는지 확인하세요.

---

## 설치

한 줄로 설치 (신규·기존 프로젝트 공통):
...
**설치되는 것**:
- `.claude/commands/` — 슬래시 명령 `/harness-plan`, `/harness`, `/review`
- `.claude/settings.json` — Stop/PreToolUse 훅 (기본값 `npm run` 형태, 프로젝트에 맞게 수정 필요)
- `scripts/execute.py` — Stage 3 오케스트레이터
- `docs/`, `phases/` — 빈 디렉토리 (`.gitkeep` 포함)
- `CLAUDE.md` — 템플릿 (없을 때만 생성)
- `.git/` — `git init` (없을 때만)

이미 있는 파일은 덮어쓰기 전에 `y/N` 프롬프트로 묻는다 (idempotent).

---

### 시나리오 A — 신규 프로젝트 (빈 레포에서 시작)
...
**설치 후**:
1. `CLAUDE.md` 템플릿을 프로젝트 규칙에 맞게 수정 (스택, CRITICAL 규칙, 명령어)
2. `.claude/settings.json`의 훅 커맨드가 `npm run ...`이니, 언어에 맞게 교체 (아래 참고)
3. Claude Code 세션에서 `/harness-plan mvp`로 첫 phase 시작

---

### 시나리오 B — 기존 프로젝트 (코드·git·CLAUDE.md 이미 있음)
...
동일한 스크립트. 기존 파일은 건드리지 않는다:
- `git init` 건너뜀 (`.git/` 이미 있으므로)
- `CLAUDE.md` 건너뜀 (이미 있으므로)
- `docs/`, `phases/`가 이미 있으면 그대로 둠

**설치 후 반드시 점검할 3가지**:

#### 1. `.claude/settings.json`의 훅 커맨드 수정 (⚠ 중요)

기본값이 `npm run lint && npm run build && npm run test`로 하드코딩돼 있다. 프로젝트 스택에 맞게 교체:

| 스택 | 교체할 커맨드 예시 |
|------|-----------------|
| Node.js/JS | (기본값 그대로 또는 `pnpm`/`yarn`으로 변경) |
| Python | `ruff check . && mypy . && pytest` |
| Go | `golangci-lint run && go build ./... && go test ./...` |
| Rust | `cargo clippy && cargo build && cargo test` |
| 기타 | 실제 빌드/테스트 커맨드 |

이 훅은 **모든 Claude Code 세션 종료 시** 실행된다 — Stage 3에서 매 step마다 AC 검증의 핵심이 되므로 반드시 맞춰야 한다.

#### 2. 기존 `CLAUDE.md` 보강

스크립트가 기존 파일을 건드리지 않았으므로, 다음 항목이 빠져있으면 직접 추가:
...
`## 아키텍처 규칙` 섹션의 CRITICAL 항목들도 기존 컨벤션에 맞게 추가하면 하네스가 더 정확히 가드레일로 활용한다.

#### 3. 기존 테스트가 통과하는 깨끗한 상태인지 확인

```bash
<프로젝트의 테스트 커맨드>
```

하네스 Stop 훅이 **매 step마다** 이 커맨드를 돌린다. 시작 전에 깨끗한 상태(전체 통과)여야 회귀를 정확히 감지할 수 있다.

**첫 phase 선택**:

| 상황 | 추천 모드 |
|------|----------|
| 새 기능 추가 | `/harness-plan feature` |
| 버그 수정 | `/harness-plan patch` |
| 구조 개선 (동작 보존) | `/harness-plan refactor` |
| `docs/`(PRD/ARCHITECTURE/ADR)가 비어있고 기존 아키텍처 문서화부터 하고 싶다면 | 작은 feature 먼저 돌려 Phase B 분석 결과로 ARCHITECTURE/ADR 축적. 또는 직접 docs/ 초안 작성 후 `/harness feature` 실행 |

**절대 `/harness-plan mvp`를 쓰지 말 것** — 기존 코드가 있는 상태에서 mvp 모드는 처음부터 다시 빌드를 시도하므로 충돌 위험.

실전 대화 예시는 [SCENARIO.md](SCENARIO.md)의 시나리오 2(feature), 3(patch) 참고.
...
```bash
git clone https://github.com/itchain/harness_framework.git /tmp/hf
cd <your-project>
cp -r /tmp/hf/.claude /tmp/hf/scripts ./
mkdir -p docs phases
# 필요 시: CLAUDE.md 작성, git init
rm -rf /tmp/hf
```

### 업데이트

하네스가 업데이트되면 프로젝트 루트에서 설치 커맨드를 다시 실행 — `y/N` 프롬프트로 `.claude/`와 `scripts/execute.py`만 선택적으로 덮어쓴다. `CLAUDE.md`와 `docs/`, `phases/`는 건드리지 않는다.
...
- [SCENARIO.md](SCENARIO.md) — **실전 사용 시나리오** (명령·답변·출력 구체 대화 예시)
